...
const TrackingTimeline = ({ items, className }: TrackingTimelineProps) => {
  ...
  return (
    <motion.ol
      className={cn("relative border-l-2 border-border ml-4", className)}
      initial="hidden"
      animate="visible"
    ...
    >
      {items.map((item, index) => (
        ...
        >
          <span
            className={cn(
              "absolute -left-[17px] flex h-8 w-8 items-center justify-center rounded-full ring-4 ring-background",
              {
