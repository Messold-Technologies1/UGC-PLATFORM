...
/** Lighter include for admin pending-approval queue (signup fields only). */ const pendingCreatorApprovalInclude = {
    user: {
        select: {
            phone: true,
            phoneVerified: true
...
let CreatorProfileService = class CreatorProfileService {
   ...
   */ async createCreatorProfileInTransaction(tx, userId, input) {
        ...
        const facetInputs = input.categorySlugs.map((slug)=>({
                dimension: _client.CreatorFacetDimension.CONTENT_CATEGORY,
                slug
            }));
    ...
    async listSuggestedCreators(anchorCreatorId) {
        ...
        if (approvalStatus !== _client.ApprovalStatus.APPROVED) {
            throw new _common.NotFoundException('Creator not found');
        }
        const categorySlugs = (anchor.facetSelections ?? []).map((row)=>row.option?.slug).filter((s)=>typeof s === 'string' && s.length > 0);
    ...
    mapSuggestedCreatorListItemDto(profile) {
        const pkg = Array.isArray(profile.packages) ? profile.packages[0] : null;
        const rawPrice = pkg?.priceAmount;
        let priceAmount = null;
        if (rawPrice !== undefined && rawPrice !== null) {
            priceAmount = typeof rawPrice?.toString === 'function' ? rawPrice.toString() : String(rawPrice);
        }
        const contentCategories = Array.isArray(profile.facetSelections) ? profile.facetSelections.map((row)=>({
                slug: String(row?.option?.slug ?? ''),
                label: String(row?.option?.label ?? '')
            })).filter((x)=>x.slug) : [];
        const v = Array.isArray(profile.portfolioVideos) ? profile.portfolioVideos[0] : null;
    ...
    mapPendingCreatorApprovalListItem(profile) {
        const dob = profile.dateOfBirth ? new Date(profile.dateOfBirth) : null;
        const age = dob && !Number.isNaN(dob.getTime()) ? (0, _creatorageutil.computeAgeYears)(dob) : null;
        const contentCategories = (profile.facetSelections ?? []).filter((row)=>row.option?.dimension === _client.CreatorFacetDimension.CONTENT_CATEGORY).map((row)=>({
                slug: row.option?.slug ?? '',
                label: row.option?.label ?? ''
            })).filter((c)=>c.slug.length > 0);
        const portfolioVideos = (profile.portfolioVideos ?? []).map((v)=>({
                id: v.id,
                creatorId: v.creatorId,
                videoUrl: v.videoUrl,
                thumbnailUrl: v.thumbnailUrl ?? null,
        ...
        return {
            ...
            contentCategories,
            portfolioVideos,
            approvalStatus: profile.creatorApproval?.status ?? _client.ApprovalStatus.PENDING,
    ...
    async listPendingCreatorApprovals(query) {
        ...
        const [total, items] = await this.prisma.$transaction([
            ...
            this.prisma.creatorProfile.findMany({
                ...
                include: pendingCreatorApprovalInclude
            })
        ]);
        return {
