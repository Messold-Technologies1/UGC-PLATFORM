...
} from "@/features/orders/api/get-brand-orders";
import {
  type CreatorOrderListItem,
} from "@/features/orders/api/get-creator-orders";
import type {
  OrderBrandSnapshot,
...
import { useGetCreatorOrdersQuery } from "@/features/orders/hooks/use-get-creator-orders-query";
import {
...
function mapBrandOrder(item: BrandOrderListItem): OrderConversation {
  ...
  return {
    ...
    avatarUrl: item.creator.profileImageUrl,
    avatarColor: colorForId(item.creator.id),
    status: item.order.status,
    subtitle: item.order.packageNameSnapshot,
    lastMessage: `${item.order.packageNameSnapshot} • Order #${shortOrderId(item.order.id)}`,
    lastMessageTime: formatConversationTime(item.order.updatedAt),
    unreadCount: 0,
    order: item.order,
...
function mapCreatorOrder(item: CreatorOrderListItem): OrderConversation {
...
function MessagingInterfaceInner({ role }: MessagingInterfaceProps) {
  const searchParams = useSearchParams();
  const orderIdParam = searchParams.get("orderId");
  const [selectedConversationId, setSelectedConversationId] = useState<string | null>(orderIdParam);

  useEffect(() => {
    if (orderIdParam) {
      setSelectedConversationId(orderIdParam);
    }
  }, [orderIdParam]);

  const brandOrdersQuery = useGetBrandOrdersQuery(
    { page: 1, limit: 50 },
    { enabled: role === "brand" },
  );
  const creatorOrdersQuery = useGetCreatorOrdersQuery(
    { page: 1, limit: 50 },
  ...
  const conversations = useMemo(() => {
    if (role === "brand") {
      return (brandOrdersQuery.data?.items ?? []).map(mapBrandOrder);
    }

    return (creatorOrdersQuery.data?.items ?? []).map(mapCreatorOrder);
  }, [brandOrdersQuery.data?.items, creatorOrdersQuery.data?.items, role]);
  ...
  const selectedConversation =
    ...
    null;
  const isLoading =
    role === "brand" ? brandOrdersQuery.isPending : creatorOrdersQuery.isPending;
  const error =
    role === "brand" ? brandOrdersQuery.error : creatorOrdersQuery.error;
