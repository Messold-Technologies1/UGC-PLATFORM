...
export function BrandRegisterForm() {
  ...
  /* Purely for the Continue button's appearance — handleNextStep still does
     the real per-step validation via form.trigger(). */
  const stepOneReady = !STEP_FIELDS[0].some((f) =>
    signupBlockers.includes(SIGNUP_FIELD_LABELS[f] ?? ""),
  );
      ...
      <div className={cn("flex-col gap-[22px]", onStepOne ? "flex" : "hidden")}>
        <div>
          ...
          {form.formState.errors.email ? (
            <p className="mt-[7px] text-xs text-red-500">
