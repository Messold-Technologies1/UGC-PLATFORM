...
export default function BrandCreditsPage() {
  ...
  const filteredTx = transactions.filter((t) =>
    inPeriod(t.createdAt, txPeriod),
  );
  const txTotalPages = Math.max(1, Math.ceil(filteredTx.length / PAGE_SIZE));
  const txClampedPage = Math.min(txPage, txTotalPages);
  const txRows = filteredTx.slice(
    (txClampedPage - 1) * PAGE_SIZE,
    txClampedPage * PAGE_SIZE,
  );

  const wdTotalPages = Math.max(1, Math.ceil(withdrawals.length / PAGE_SIZE));
  const wdClampedPage = Math.min(wdPage, wdTotalPages);
  const wdRows = withdrawals.slice(
    (wdClampedPage - 1) * PAGE_SIZE,
    wdClampedPage * PAGE_SIZE,
  );
  ...
  return (
    <div className="w-full space-y-6 px-4 py-8 sm:px-6 lg:px-8 xl:px-10 2xl:px-12">
      {/* Balance hero card */}
      ...
      {/* Transaction history + Refund requests, side by side */}
          ...
          {filteredTx.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-12 text-center">
              ...
              <p className="text-sm font-medium text-foreground">
                No transactions{txPeriod === "all" ? " yet" : " in this period"}
              </p>
              {txPeriod === "all" && (
        ...
        <div className="rounded-3xl border border-border/60 bg-card p-6 shadow-sm">
          ...
          {withdrawals.length === 0 ? (
            <div className="flex flex-col items-center gap-2 py-12 text-center">
              ...
              <p className="max-w-xs text-xs text-muted-foreground">
                Request a refund to move your credits back to your bank.
              </p>
            </div>
          ) : (
            <>
                  ...
                  >
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      ...
                      {w.status === "REQUESTED" && (
                        <button
                          type="button"
                          className="rounded-lg border border-border px-2.5 py-1 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted disabled:opacity-50"
