 ...
 * can't quietly kill it, and `keepAlive` also enables detection of a
 * half-open connection so ioredis reconnects instead of hanging deaf.
 */
export function buildBullmqConnection(redisUrl: string): ConnectionOptions {
  const opts: ConnectionOptions = {
    ...
    // Send a TCP keep-alive probe after 30s of idle so managed Redis / the
    // network path doesn't drop the worker's blocking connection.
    keepAlive: 30_000,
    // Fail a request quickly if the socket has gone half-open, so a stale
    // blocking connection surfaces and reconnects rather than silently hanging.
    noDelay: true,
